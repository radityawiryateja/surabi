<template>
  <section id="menu" class="bg-brand-surface dark:bg-brand-surfaceDark py-16 md:py-20 relative">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <!-- Filter Tabs -->
      <div
        class="flex justify-start sm:justify-center gap-2 md:gap-3 mb-10 overflow-x-auto hide-scroll pb-2"
        data-aos="fade-up"
        data-aos-delay="100"
      >
        <button
          ...
          :key="cat"
          class="px-6 py-2.5 md:px-8 md:py-3 rounded-full text-xs md:text-sm font-bold whitespace-nowrap shadow-md transition-all"
          :class="
            activeFilter === cat
      ...
      <div v-if="isLoading" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
        ...
        >
          ...
          <div class="p-4 md:p-6 space-y-3">
            <div class="h-5 bg-gray-200 dark:bg-gray-700 rounded-full w-3/4"></div>
      ...
      <!-- Load More -->
      <div v-if="hasMore && !isLoading" class="text-center mt-10 md:mt-12" data-aos="fade-up">
        <button
...
// Defined locally so MenuCatalog.vue stays a single file.
  ...
  setup(props, { emit }) {
    return () => {
      ...
      return h(
        ...
        {
          class:
            'group cursor-pointer bg-brand-light dark:bg-brand-dark rounded-3xl md:rounded-4xl overflow-hidden shadow-sm hover:shadow-2xl hover:-translate-y-1 md:hover:-translate-y-2 transition-all duration-300',
          'data-aos': 'fade-up',
        ...
        [
          ...
          h('div', { class: 'p-4 md:p-6' }, [
            h(
              ...
              {
                class:
                  'font-serif font-bold text-lg md:text-xl mb-1 md:mb-2 text-brand-primary dark:text-white group-hover:text-brand-accent transition-colors',
              },
              item.nama_produk
            ),
            h(
              ...
              {
                class:
                  'text-[10px] md:text-sm text-gray-500 dark:text-gray-400 mb-3 md:mb-4 line-clamp-2 leading-relaxed',
              },
              item.deskripsi
            ),
            h(
              'p',
              { class: 'font-bold text-brand-primary dark:text-brand-primaryDark text-base md:text-xl' },
